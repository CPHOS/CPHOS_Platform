...
> 调研对象：CPHOS 联考旧平台（ThinkCMF / MySQL 5.7.34-log；旧库主机、端口与库名已脱敏）
...
> 原始探针输出与脚本保存在私有调研环境，未纳入公开仓库；本报告保留整理后的统计结论。
...
- `cmf_user`：2 个管理员（用户名已脱敏），ThinkCMF 标准字段，密码 `cmf_password` 哈希（算法未验证）；`cmf_user_token` 会话 token 2 条（无需保留）。
- `cmf_asset`：文件元数据（file_key/md5/sha1/file_path…）。`subject.image` 存相对对象键（如 `<旧对象前缀>/default/20260627/xxx.jpg`），与 `asset.file_path` 字符串精确匹配形成可选关系；第30届 2520 键中 2331 个可匹配。
...
- 第30届 313 套：312 套 `student.user_id = test_paper.user_id`（其中 1 套学生学校与上传者学校不一致：学生 S1 挂"个人"学校 134、上传者属学校 413），另 1 套 user_id 也不一致（卷 13904：学生 S2 属教练（member.id=2846）、上传者（member.id=149））。
...
- 图片字段：13 行 NULL、650 行空串、137,499 行带旧对象前缀（137,352 个不同对象键、0 条完整 URL）；完整访问域名在未提供的网站/存储配置中。
...
管理员 AI 的《CPHOS 联考系统现有数据结构与业务逻辑说明》收到后，其关键论断均已在旧库只读复核。文档引用的本地源码与 `30th/` 导出物未公开，核对以文档文本与数据库实测结论为准。
...
本报告结论来自服务器管理员提供的业务说明、旧库只读查询（仅 SELECT / SHOW / information_schema，抽样 LIMIT 5）和线上后台走查。原始探针脚本与输出保存在私有环境，公开仓库仅保留整理后的统计结论与脱敏后的数据特征。

## 附录 B：敏感信息处理说明

公开版本已对旧库主机、管理后台地址、管理员用户名，以及抽样中出现的学生/教练姓名做脱敏；原始只读探针输出仍可能含未脱敏字段，仅保存在私有环境，未纳入本仓库。
