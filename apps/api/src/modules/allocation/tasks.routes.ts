...
import { getMarkingTaskImageStream, getMarkingTaskPageStream } from '../papers/papers.service.js';
...
export async function taskRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/tasks/:taskId/images/:imageId/file', { onRequest: guard }, async (req, reply) => {
    const { taskId, imageId } = req.params as { taskId: string; imageId: string };
    const file = await getMarkingTaskImageStream(
      BigInt(req.user.sub),
